...
class TrainerRoad:
    ...
    def connect(self):
        ...
        r = self._session.post(self._login_url, data=data, allow_redirects=False)
    ...
    def _read_profile(self):
        ...
        if r.status_code != 200:
            raise RuntimeError(
                "Error getting profile from TrainerRoad (Code {})".format(r.status_code)
            )

        profile_data = r.json()
        logger.debug(
            "Profile API response: {}".format(json.dumps(profile_data, indent=2))
        )
    ...
    def _write_profile(self, new_values):
        ...
        logger.info(
            "Current profile values: Weight={}, FTP={}".format(
        ...
        logger.info(
            "Updating profile: Weight={}, FTP={}".format(
                data.get(self._weight), data.get(self._ftp)
            )
        )
        ...
        logger.debug(
            "PUT request status: {}, response: {}".format(r.status_code, r.text[:200])
        )

        if r.status_code not in [200, 204]:
            raise RuntimeError(
                "Error updating TrainerRoad profile (Code {})".format(r.status_code)
            )

        # Verify the changes
        updated_data = self._read_profile()
        logger.info(
            "Profile updated successfully: Weight={}, FTP={}".format(
        ...
        for key in self._numerical_verify:
            if key in new_values:
                ...
                if (
                    key == self._weight
                    and original_data.get("Units") == self._units_imperial
                ):
                ...
                if abs(expected - actual) > 0.1:  # Allow small rounding differences
                    raise RuntimeError(
                        "Failed to verify numerical key {}: expected {}, got {}".format(
                            key, expected, actual
                        )
                    )
    ...
    def download_tcx(self, id):
        ...
        if res.status_code != 200:
            raise RuntimeError("Unable to download (code = {})".format(res.status_code))
